	...
	*/
    ...
    for (int iEvent=0; iEvent<n_event; iEvent++) {     
        if(feof(infile1)) {
            ...
            output_filename3 = "successed.dat";// output files of final hadrons
                ...
                if (pythia.event[i].isFinal() ){
                    ...
                    if (sss) {
                        ...
                        */
                        ...
                        Vec4 pCoM = pythia.event[m1index].p() +  pythia.event[m2index].p();
                        if (pCoM.m2Calc() < 0) {
                            pCoM = pythia.event[m1index].p();
                            if ( pythia.event[m1index].e() < pythia.event[m2index].e()) pCoM =  pythia.event[m2index].p();
                        }
                        pCoM = pythia.event[i].p();
                        ...
                        Vec4 formation_4 = {1.0* phadron.px()/phadron.e() + local_pos.px(), 
                                            ...
                                            1.0+ local_pos.e()};
                       /*
                       Vec4 formation_4 = {hbarc* phadron.px()/hmt,//  + local_pos.px(), 
                                            hbarc*phadron.py()/hmt,//+ local_pos.py(), 
                                            hbarc*phadron.pz()/hmt,// + local_pos.pz(), 
                                            hbarc*phadron.e()/hmt}; 
                       *//* 
                        Vec4 formation_4 = {hbarc* phadron.px()/hmt,//  + local_pos.px(), 
                                            hbarc*phadron.py()/hmt,//+ local_pos.py(), 
                        ...
                        */
                        /*
                        Vec4 formation_4 = {0. +local_pos.px() , 0.+local_pos.py(), 0.+local_pos.pz(),
                                             1.+local_pos.e()};
                        */
                        ...
                        if (ttpo[m1index] > ttpo[m2index]) {
                            x_hadron = (xxpo[m1index] * epo[m1index] + (xxpo[m2index] + delta_t * pxpo[m2index]/epo[m2index]) * epo[m2index])/ (epo[m1index] + epo[m2index])
                                   + hbarc*cbar_meson_px/hmt;
                            y_hadron = (yypo[m1index]* epo[m1index] + (yypo[m2index] + delta_t * pypo[m2index]/epo[m2index])* epo[m2index])/(epo[m1index] + epo[m2index])
                                   + hbarc*cbar_meson_py/hmt;
                            z_hadron = (zzpo[m1index]* epo[m1index] + (zzpo[m2index] + delta_t * pzpo[m2index]/epo[m2index])* epo[m2index])/(epo[m1index] + epo[m2index])
                                   + hbarc*cbar_meson_pz/hmt;
                            t_hadron = ttpo[m1index] +  hbarc*cbar_meson_energy/hmt;
                        } else {
                            x_hadron =( xxpo[m2index]* epo[m2index] + (xxpo[m1index] + delta_t * pxpo[m1index]/epo[m1index])* epo[m1index])/(epo[m1index] + epo[m2index])
                                   + hbarc*cbar_meson_px/hmt;
                            y_hadron = (yypo[m2index]* epo[m2index] + (yypo[m1index] + delta_t * pypo[m1index]/epo[m1index])* epo[m1index])/(epo[m1index] + epo[m2index])
                            ...
                            z_hadron = (zzpo[m2index]* epo[m2index] + (zzpo[m1index] + delta_t * pzpo[m1index]/epo[m1index])* epo[m1index])/(epo[m1index] + epo[m2index])
                                   + hbarc*cbar_meson_pz/hmt;
                            t_hadron = ttpo[m2index];// +  hbarc*cbar_meson_energy/hmt;
                        ...
                        */
                        ...
                        if (ttpo[m1index] > ttpo[m2index]) {
                        xcenter  = (xxpo[m1index] * epo[m1index] + (xxpo[m2index] + delta_t * pxpo[m2index]/epo[m2index]) * epo[m2index])/ (epo[m1index] + epo[m2index]);
                        ycenter = (yypo[m1index]* epo[m1index] + (yypo[m2index] + delta_t * pypo[m2index]/epo[m2index])* epo[m2index])/(epo[m1index] + epo[m2index]);
                        zcenter = (zzpo[m1index]* epo[m1index] + (zzpo[m2index] + delta_t * pzpo[m2index]/epo[m2index])* epo[m2index])/(epo[m1index] + epo[m2index]);
                        tcenter = ttpo[m1index];
                        } else {
                        xcenter = (xxpo[m2index]* epo[m2index] + (xxpo[m1index] + delta_t * pxpo[m1index]/epo[m1index])* epo[m1index])/(epo[m1index] + epo[m2index]);
                        ycenter = (yypo[m2index]* epo[m2index] + (yypo[m1index] + delta_t * pypo[m1index]/epo[m1index])* epo[m1index])/(epo[m1index] + epo[m2index]);
                        zcenter = (zzpo[m2index]* epo[m2index] + (zzpo[m1index] + delta_t * pzpo[m1index]/epo[m1index])* epo[m1index])/(epo[m1index] + epo[m2index]);
                        tcenter = ttpo[m2index];
                        }
                        
                        Vec4 pCoM = pythia.event[i].p();
                        
                        Vec4 xcom = {xcenter, ycenter, zcenter, tcenter};
                        //   Vec4 xcom = {0.0, 0.0, 0.0, 0.0};
                        xcom.bstback(pCoM);
                        double newt = xcom.e() + pythia.event[i].m();// Assuming the formation time is proportion to its mass.
                        xcom =  {xcom.px(), xcom.py(), xcom.pz(), newt};
                        xcom.bst(pCoM);
                        ...
                        */
                        ...
                        //if (abs(x_hadron) > 50.) x_hadron = 50.0;
                        //if (abs(y_hadron) > 50.) y_hadron = 50.0;
                        //if (abs(z_hadron) > 50.) z_hadron = 50.0;
                        //if (abs(t_hadron) > 50.) t_hadron = 50.0;
